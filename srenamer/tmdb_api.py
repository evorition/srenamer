...
class MovieDatabase:
    ...
    def choose_series(self, series):
        ...
        if counter:
            for i in range(20 if counter > 20 else counter):
                print(str(i) + ".", series_list[i]['first_air_date'], "-", 
    ...
    def list_episodes(self, series_id):
        ...
        r_json = requests.get(self.base_uri + 'tv/' + str(series_id) + '/season/' + str(season), 
                params=params).json()
        episodes_list_raw = r_json['episodes']
        self.episodes = ['{0}. {1}'.format(episodes_list_raw[i]['episode_number'],
